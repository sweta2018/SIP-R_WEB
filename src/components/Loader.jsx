...
export default function Loader({ onComplete }) {
  ...
  useEffect(() => {
    ...
    const timePerFrame = 0.35; // reduced time to read
    const fadeDuration = 0.2; // faster crossfade
    ...
    steps.forEach((_, index) => {
      ...
      if (index === 4) {
        // Wait briefly, then fade out the step4 image completely
        tl.to(img, { opacity: 0, duration: 0.2 }, `crossfade${index}+=0.4`)
        tl.to(textNode, { opacity: 0, duration: 0.2 }, `crossfade${index}+=0.4`)

        const logoLetters = [sRef.current, glassRef.current, pRef.current, dropRef.current, rRef.current];
        tl.fromTo(logoLetters,
          { y: -60, opacity: 0 },
          { y: 0, opacity: 1, duration: 0.4, stagger: 0.05, ease: 'bounce.out' },
          `crossfade${index}+=0.5`
        )
      }
      ...
      if (index === 4) {
        tl.to({}, { duration: 0.1 }, `crossfade${index}+=1.2`)
      } else {
